...
public partial class Login : Page
{
    ...
    protected void btnLogin_Click(object sender, EventArgs e)
    {
        ...
        // FullName falls back to Username when the account has no linked
        // CmsStaff record (e.g. an Admin account not tied to a staff row).
        const string query = @"
            SELECT ua.UserID,
            ...
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        {
                            // Optional: set a persistent auth cookie here if you want
                            // "remember me" to survive browser restarts.
                        }
        ...
        catch (Exception ex)
        {
            ...
            // TODO: log ex to your error/audit log table instead of swallowing it.
        }
    }
    ...
    private void RedirectToDashboard(string role)
    {
        switch (role)
        {
            case "Admin":
                ...
                break;
            case "Supervisor":
                Response.Redirect("~/DashboardSupervisor.aspx");
                break;
            case "Staff":
    ...
    // Must exactly match the hashing used in SeedAdminUser.sql:
